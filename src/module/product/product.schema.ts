...
export const CreateProductSchema = z.object({
    ...
    price: z.number().min(1000, 'product:price_min_1000').optional(),
    quantity: z.number().int().nonnegative('product:quantity_negative').optional(),
...
}).refine((data) => Object.keys(data).length > 0, { message: 'product:update_empty' }).strict().openapi('UpdateProductRequest');

export const FilterProductQuerySchema = z.object({
    ...
    status: ProductStatusSchema.optional(),
}).strict().openapi('FilterProductQuery');

export const AddProductImageSchema = z.object({
    product_id: z.string().uuid(),
    image_url: z.string().url(),
    is_main: z.preprocess((val) => val === 'true' || val === true, z.boolean()).default(false),
}).strict().openapi('AddProductImageRequest');

export const CreateProductVariantSchema = z.object({
    product_id: z.string().uuid(),
    name: z.string().trim().min(1).max(100),
}).strict().openapi('CreateProductVariantRequest');

export const CreateProductVariantOptionSchema = z.object({
    variant_id: z.string().uuid(),
    value: z.string().trim().min(1).max(100),
}).strict().openapi('CreateProductVariantOptionRequest');
...
}).strict().openapi('CreateProductStockRequest');

export const UpdateProductStockSchema = z.object({
    sku: z.string().trim().regex(/^[A-Z0-9-_.]+$/).max(100).optional(),
    price: z.number().min(1000).optional(),
    quantity: z.number().int().nonnegative().optional(),
}).strict().openapi('UpdateProductStockRequest');

export const ProductImageResponseSchema = z.object({
    id: z.string().uuid(),
    image_url: z.string(),
    is_main: z.boolean(),
}).strict().openapi('ProductImageResponse');

export const ProductVariantOptionResponseSchema = z.object({
    id: z.string().uuid(),
    value: z.string(),
}).strict().openapi('ProductVariantOptionResponse');

export const ProductVariantResponseSchema = z.object({
    id: z.string().uuid(),
    name: z.string(),
    options: z.array(ProductVariantOptionResponseSchema).optional(),
}).strict().openapi('ProductVariantResponse');
...
export const ProductStockResponseSchema = z.object({
    ...
    quantity: z.number(),
    option_ids: z.string(),
}).strict().openapi('ProductStockResponse');
...
export const ProductResponseSchema = z.object({
    ...
    created_at: z.date(),
    updated_at: z.date(),
}).strict().openapi('ProductResponse');
...
}).strict().openapi('ProductListResponse');
...
}).strict().openapi('CreateReviewRequest');

export const UpdateReviewSchema = z.object({
    rating: z.number().int().min(1).max(5).optional(),
    comment: z.string().trim().optional(),
    images: z.string().trim().optional(),
}).refine((data) => Object.keys(data).length > 0, { message: 'review:update_empty' }).strict().openapi('UpdateReviewRequest');
...
export const QuestionResponseSchema = z.object({
    ...
    created_at: z.date(),
    updated_at: z.date(),
}).strict().openapi('QuestionResponse');
...
}).strict().openapi('ReviewListResponse');
...
}).strict().openapi('QuestionListResponse');