...
- Hạ tầng theo `docker-compose.yaml`: backend (port 8888), PostgreSQL, Redis + Redis Commander, pgAdmin, MinIO (9000/9001), Mailhog (8025), Nginx (reverse proxy cho backend, seller web và admin), Certbot.
- API REST tại `/api/v1`, response đồng nhất qua `utils/response`; hỗ trợ i18n trong `server/src/locales`.

## Dự án dùng làm gì
- Nền tảng marketplace 2 phía (người bán / người mua) có portal seller riêng và app mobile cho khách hàng.
